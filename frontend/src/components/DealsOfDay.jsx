 ...
 */
export default function DealsOfDay({ limit = 8 }) {
  // choose top deals: verified and highest discount first
  const deals = dummyCoupons
    .slice()
    .filter(c => new Date(c.expiryDate) > new Date()) // only active
    .sort((a, b) => {
      const va = a.discountType === "percent" ? a.discountValue : (a.discountValue / 100);
      const vb = b.discountType === "percent" ? b.discountValue : (b.discountValue / 100);
      return vb - va;
    })
    .slice(0, limit);
    ...
    <section className="my-6">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-sm text-gray-500">Hand-picked offers just for you</div>
        </div>
            ...
            {deals.map(deal => (
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <button
                    onClick={() => window.dispatchEvent(new CustomEvent("showDeal", { detail: { couponId: deal._id } }))}
                    className="px-3 py-1 bg-orange-500 text-white rounded text-sm"
                  >
                    View
                  </button>
                  <div className="text-xs text-gray-400">Expires: {new Date(deal.expiryDate).toLocaleDateString()}</div>
                </div>
              </div>
            ))}
          </div>
